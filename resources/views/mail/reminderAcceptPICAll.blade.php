<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Approved Department Head PIC</title>
</head>
<body style="color: black; font-family: Arial, sans-serif;">

    <h2 style="font-weight: bold;">Approved Department Head PIC</h2>

    <p style="font-size: 16px;">
        Department Head PIC Area <strong>{{ $data[0]['area']->name }}</strong> already approved perbaikan temuan patrol pada tanggal patroli <strong>{{ $data[0]['laporan_patrol']['tanggal_patrol'] }}</strong> dengan detail berikut.
    </p>
    @foreach ($data as $item)
    <ul>
        <li>
            <p style="font-size: 16px;">Temuan : {{ $item['temuan'] }}</p>
        </li>
        @if($item['rank'] == 'A') <li> <p> Dengan status rank {{ $item['rank'] }} </p></li> @endif
        <li>
            <p style="font-size: 16px;">Link temuan : {{ config('app.link_website') }}/detail/{{ $item['id'] }}</p>
        </li>
    </ul>
    <br>
    @endforeach

    <p style="font-size: 16px;">Link laporan Patroli Area : {{ config('app.link_website') }}/patrolEHS/{{ $data[0]['id'] }}</p>
<br>
<br>
<br>
    <p style="font-size: 16px;">Terima kasih.</p>

</body>
</html>
