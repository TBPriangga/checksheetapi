            ...
            <div class="ibox">
                ...
                <div class="ibox-content">
                    ...
                    <div class="row g-2 align-items-center mb-3">
                        ...
                        <!-- Export All (TANPA TAB) -->
                        <div class="col-sm-2 col-6">
                            <button type="button" id="download-all-btn" class="btn btn-sm btn-danger w-100">
                                Export All
                            </button>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Filter & Reset
    $('#filter-button').on('click', () => table.ajax.reload());
    $('#reset-filter').on('click', function () {
        $('#area-filter').val('');
        $('#npk-filter').val('');
        table.ajax.reload();
    });
    $('#area-filter, #npk-filter').on('change', () => table.ajax.reload());

    // DOWNLOAD PDF (FILTERED) - TANPA TAB
    $('#download-pdf-btn').on('click', function () {
        const area = $('#area-filter').val() || '';
        const npk = $('#npk-filter').val() || '';
        const url = new URL('{{ route("exportLaporanPdf") }}');
        if (area) url.searchParams.set('area', area);
        if (npk) url.searchParams.set('npk', npk);
        ...
        .then(r => {
            if (!r.ok) throw new Error('Gagal download');
            const name = r.headers.get('Content-Disposition')?.match(/filename="?([^"]+)"?/)?.[1] || 'Laporan.pdf';
        ...
        .finally(() => btn.prop('disabled', false).html(text));
    });

    // EXPORT ALL - TANPA TAB
    $('#download-all-btn').on('click', function () {
        const url = '{{ route("exportLaporanPdf") }}';
        const btn = $(this);
        const text = btn.html();
        btn.prop('disabled', true).html('Downloading...');

        fetch(url)
        .then(r => r.blob().then(b => ({ b, name: r.headers.get('Content-Disposition')?.match(/filename="?([^"]+)"?/)?.[1] || 'Laporan_All.pdf' })))
        .then(({ b, name }) => {
            const a = document.createElement('a');
            a.href = URL.createObjectURL(b);
            a.download = name;
