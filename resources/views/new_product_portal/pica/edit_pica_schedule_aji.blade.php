			...
			<div class="ibox-content">
				<form method="POST" action="{{ route('NewProductPortalPicaScheduleAjiController.update', $data->id) }}" enctype="multipart/form-data">
					...
					<div class="form-group  row"><label class="col-sm-2 col-form-label">Type</label>
						<div class="col-sm-10"><input type="text" name="type" class="form-control" value="{{$data->type}}" required disabled></div>
					</div>
                    <div class="hr-line-dashed"></div>
					...
					{{-- <div class="form-group row"><label class="col-sm-2 col-form-label">Dept</label>
						<div class="col-sm-10">
	                        <select id="model-dropdown" class="select2 form-control m-b" name="dept"  disabled>
					...
					<div class="form-group row"><label class="col-sm-2 col-form-label">PIC</label>
						<div class="col-sm-10">
	                        {{-- <select id="model-dropdown" class="select2 form-control m-b" name="pic"  disabled>
								<option value="-">-</option>
	                        	@foreach ($usersWithAPQPRole as $key => $user)
									@if($user->department->code." | ".$user->email == $data->pic)
										<option value="{{$user->department->code." | ".$user->email}}" selected>{{$user->department->code." | ".$user->email}}</option>
									@else
										<option value="{{$user->department->code." | ".$user->email}}">{{$user->department->code." | ".$user->email}}</option>
									@endif
								@endforeach
	                        </select> --}}
					...
					<div class="form-group  row"><label class="col-sm-2 col-form-label">Due date </label>
						<div class="col-sm-10"><input type="date" name="due_date" class="form-control" value="{{$data->due_date ? date("Y-m-d", strtotime($data->due_date)) : null}}" readonly ></div>
					</div>
					<div class="hr-line-dashed"></div>
